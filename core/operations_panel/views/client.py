...
from core.operations_panel.forms.client import ClientForm
from core.operations_panel.models.client import Client
from core.system.views import AdminListView


class ClientListView(AdminListView):
    model = Client
    form = ClientForm
    template_name = 'base/elements/views/datatable_list.html'
    datatable_headers = ["Nombre", "Razón Social", "RFC", "Email", "Teléfono", "Régimen Fiscal"]
    datatable_keys = ["name", "business_name", "rfc", "email", "phone", "tax_regime"]
    ...
    def handle_update(self, request, data):
        ...
        if not form.is_valid():
            data['error'] = form.errors
        elif not form_address.is_valid():
            data['error'] = form_address.errors
        else:
            instance = form.save()
    ...
    def render_form(self, request, instance, form=None):
        ...
        form_step_2 = AddressForm(instance=instance.address)
